...
module.exports = (app) => {
    app.get('/api/profile', authenticateToken, isClient, async (req, res) => {
        ...
        console.error(error);
        res.status(500).json({
            message: "Impossible de récupérer votre profil. Réessayez plus tard.",
            error
        });
        }
    })
    ...
    app.put('/api/profile', authenticateToken, async (req, res) => {
        try {
            ...
            // si nouveau username fourni, vérifier l'unicité
            ...
            res.status(500).json({ message: "Impossible de mettre à jour le profil.", error });
        }
    });
}